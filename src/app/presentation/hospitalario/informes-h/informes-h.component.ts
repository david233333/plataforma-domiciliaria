...
/**
 * Pantalla de Informes (hospitalario). SOLO DISEÑO: los catálogos son datos de
 * ejemplo y la descarga genera un CSV local (sin backend). Comparte el flujo y
 * los patrones de la pantalla no hospitalaria (consistencia entre ámbitos),
 * con el vocabulario propio del ámbito hospitalario (sede, servicios clínicos).
 *
...
export class InformesHComponent {
  ...
  /** Tipos de servicio clínico (datos de ejemplo). */
  ...
  /** Sedes hospitalarias de origen (datos de ejemplo). */
  protected readonly sedes: Opcion[] = [
  ...
  protected readonly mostrarErrorSedes = computed(() =>
    this.campoInvalido('sedes'),
  );
  ...
  protected limpiarFiltros(): void {
    this.filtros.reset({ servicios: [], sedes: [] });
  ...
  private enfocarPrimerError(): void {
    const orden = ['servicios', 'sedes', 'fechaDesde', 'fechaHasta'];
    const primero =
  ...
  private generarDescarga(informe: OpcionInforme): void {
    const { servicios, sedes, fechaDesde, fechaHasta } =
      this.filtros.getRawValue();

    const encabezado = ['Servicio', 'Sede', 'Fecha desde', 'Fecha hasta'];
    const fila = [
      this.etiquetas(this.servicios, servicios),
      this.etiquetas(this.sedes, sedes),
