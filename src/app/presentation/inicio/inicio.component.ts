 ...
 *   - `styles` scoped → SOLO lo que Tailwind no cubre bien: el chip de ícono de la
 *     lista y la sombra de apoyo de la mascota.
 */
@Component({
  selector: 'app-inicio',
  ...
  template: `
    <section class="flex justify-center px-4 py-6 animate-fade-in">
      <div
      ...
      >
        ...
        <!-- Ilustración -->
        <figure class="m-0 flex justify-center order-1 lg:order-2">
          <img
            ...
            class="inicio-tiger w-full max-w-sm h-auto"
          />
        </figure>
      </div>
    </section>
  ...
  styles: `
    ...
    /* Mascota: presencia con una sombra de apoyo discreta. Sin halo difuso ni
       flotación perpetua —registro clínico: la pantalla no se mueve sola
       (PRODUCT.md: "clinicians do not want to watch the page assemble"). La
       entrada única la aporta .animate-fade-in en la <section>, igual que el
       resto de páginas. */
    .inicio-tiger {
      filter: drop-shadow(0 8px 16px rgb(2 6 23 / 0.08));
    }
  `,
})
